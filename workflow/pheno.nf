...
process add_pcs_multipheno{
  label 'R'
  cpus params.max_plink_cores
  input :
      tuple val(pheno),path(bed), path (bim), path(fam), path(phenofile)
  publishDir "${params.output_dir}/pheno/pca/", mode:'copy'
  output :
    tuple  val(pheno),path(newfilepheno), emit :data
    tuple val(pheno), env pcs, emit : pcs_covar
  script :
     plkf=bed.baseName
...
process format_pheno_multipheno {
  label 'R'
  input :
       tuple val(pheno) path(bed), path (bim), path(fam), path(phenofile), val(covar)
  publishDir "${params.output_dir}/pheno/", mode:'copy'
  output :
       tuple env newpheno, path("${newdatafile}.pheno"), val(newcovar), emit :data
       path("${newdatafile}*")
  script :
      newdatafile=phenofile.baseName+'_format'
      covar2=(covar=="")? "" : " --covar $covar "
...
workflow wf_prepare_multipheno{
   take : 
        phenofile 
        plink
   main :
    if(params.addpcs>=1){
      add_pcs_multipheno(plink.combine(phenofile))  
      new_data=add_pcs_multipheno.out.data
      newcovar=add_pcs_multipheno.out.pcs_covar
      data_ch=add_pcs_multipheno.out.data
    }else {
    newcovar=channel.from(params.covariates)
    data_ch=phenofile
   }
  format_pheno_multipheno(plink.combine(data_ch).join(newcovar)) 
  emit :
